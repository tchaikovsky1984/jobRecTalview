...
def storer(insert_info: DBInsertData) -> bool:
    try:
        ...
        l = []
        for row in records:
            l.append((row["title"], row["company"], row["description"] if row["description"] is not None else "", row["location"], row["embedding"], row["job_url"], insert_info.search_title, insert_info.search_pref_country, insert_info.search_pref_area))

        execute_values(
                cursor,
                "INSERT INTO job (title, company, description, location, embedding, url, search_title, search_pref_country, search_pref_area) VALUES %s ON CONFLICT (url) DO NOTHING",
                l
                )
